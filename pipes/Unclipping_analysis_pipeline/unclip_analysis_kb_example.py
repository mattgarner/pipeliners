...
def directory_setup(): 
	# setus up a directory with random ID #
	tempfile.makestemp(prefix= "Unclip Analysis - ")
...
def smalt( infile1, infile2, outfile):
    ...
    return "/software/bin/smalt_0.7.6 map -f samsoft /refs/HIV/K03455_s1k6 " + infile1 + " " + infile2 + " > " + outfile
...
pipeliners.system_call('Cut Adapters', cutadapt(sample_name + ".1.fq.gz", sample_name + "_ar.1.fq.gz"))
pipeliners.system_call('Cut Adapters', cutadapt(sample_name + ".2.fq.gz", sample_name + "_ar.2.fq.gz"))
